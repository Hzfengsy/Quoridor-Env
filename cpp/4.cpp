...
std::string ai_name = "多谢，多谢乌蝇哥";	//name of AI
const int BOARDSIZE = 9;						//Size of the board
const int VERTEXSIZE = 100;						//Size of vertexes
const int DEPTH = 2;							//Depth of minimax search
...
inline double AlphaBeta(QuoridorBoard state, int player, int depth, double alpha, double beta){
	...
	//state.Show();
	//cerr << "List of actions :\n";
	//ShowList(listofactions);

	if (player == OPPONENT){
		for (auto action : listofactions){
			QuoridorBoard newstate = state; 
			newstate.MakeAction(action.second, player);
			//cerr << "---------------------player = " << player << " depth = " << depth << "--------------------" << endl;
			//state.Show();
			//cerr << "Action :" << action.second.first << " " << action.second.second << endl;
			double score = AlphaBeta(newstate, player ^ 1, depth - 1, alpha, beta);
			//cerr << "Move : " << action.second.first << " " << action.second.second << " score : " << score << endl;
			if (score < beta){
				beta = score;
				if (depth == DEPTH) bestaction = action;
...
std::pair<int, int> Action() {
	...
	double bestscore = AlphaBeta(board, MYAI, DEPTH, -INF, INF);
